...
export default class BEditor extends React.Component {
  ...
  render() {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(LinkPlugin.LinkButton, {
          editorState: editorState
        }),
        React.createElement(LinkPlugin.UnlinkButton, {
          editorState: editorState
        }),
        React.createElement('div', { className: 'BEditor-ctrl-sep' }),
  ...
  renderBlock(block) {
    return this.blockRendererFns.reduce((finalConfig, fn) => finalConfig || fn(block), null);
  }
  ...
  handlePropsChange({ html, quotes } = this.props) {
    ...
    this.handleChange(EditorState.createWithContent(contentState, this.decorator));
  }
  ...
  handleChange(nextEditorState) {
    nextEditorState = this.reducers.reduce((editorState, reduce) => reduce(editorState, {
      name: 'change',
  ...
  handleKeyCommand(command) {
    const nextEditorState = this.keyCommandHandlers.concat(RichUtils.handleKeyCommand).reduce((editorState, handleKeyCommand) =>
    // Only handle once
    editorState || handleKeyCommand(this.state.editorState, command), null);
  ...
  handleEvent(name, payload) {
    this.handleChange(this.reducers.reduce((editorState, handleEvent) => handleEvent(editorState, { name, payload }), this.state.editorState));
  }
